...
def Aligner(h_j, e_ij, e_prev=None, h_prev=None):
    
    softmax = nn.Softmax(dim=-1)
    
    # If we have looked at any previous words in source sent
    if e_prev is not None:
        e_prev.append(e_ij)
        h_prev = torch.cat(h_prev, h_j, dim=0)
        
        
        e_t = torch.tensor(e_prev)
        
        # get the softmax value for e_ij
        alphas = softmax(e_t)
        
        c_i= 0
        for i in range(len(alphas)):
            c_i += alphas[i]*h_prev[i]
        
        return torch.tensor(c_i)
    
    # If we start
    else:
        return torch.tensor(h_j*e_ij)
...
for i in range(len(ipt)):
    if i == 0:
        h_i_forward = E.forward(ipt[i], E.get_initial_hidden_state())
        h_i_backward = E.backward(ipt[i], E.get_initial_hidden_state())
